...
**1. 对话式误差分析框架**
- 传统工具多为黑盒式计算，本项目提供交互式引导，用户与 AI 一步步讨论误差来源
- LLM 自动理解自然语言公式，通过函数调用调用符号计算 MCP 服务，生成完整 LaTeX 推导过程
- 突破点：用户无需掌握 SymPy 语法，即可获得专业的误差传播分析

**2. 流式 LLM 交互与深度思考**
- 实时流式输出：误差分析与报告生成全程流式展示，包括 LLM 思考过程、MCP 符号计算调用、计算结果
- 智能模型选择：Plus 模型自动启用深度思考模式（enable_thinking），适应复杂物理推理
- 突破点：用户可实时观察 AI 思考过程，增强信任度；流式输出提升交互体验

**3. AI 安全约束与学术规范**
- 独特的反幻觉机制：系统提示词明确禁止 AI 编造数据，要求仅基于已提供的测量结果进行推理
- 对话式规划阶段：先澄清实验背景、方法、条件，后进行 Act（撰写），确保报告逻辑自洽
- 片段化输出：报告按 section/subsection 级别生成，避免完整文档结构导致的冗余，支持灵活组织
- 突破点：防止 LLM 生成虚假数据或不切实际的讨论，适配学术严谨度要求

**4. 学术规范融合**
- 符合 GB/T 15000.2 物理量与单位标准，确保报告文献价值
- A/B 类不确定度自动分离，支持加权最小二乘法与 χ² 检验，超越简单线性回归
- 自动计算参数不确定度而非仅依赖仪器精度，符合测量学教学要求
- 突破点：融合中国高校实验教学标准，提升学位论文与期刊投稿的适配性

### 竞争优势

| 功能 | 本项目 | 传统方法 | Origin/Excel |
|------|------|--------|--------------|
| **数据输入** | AI 识别拍照 | 手工 Excel | 手工 Excel |
| **误差分析** | 对话式 LLM 引导 + 符号计算 | 手算 | 有限函数库 |
| **参数不确定度** | 自动计算 | 手算 | 需插件 |
| **推导过程可视化** | LaTeX 完整推导 | 无 | 有限 |
| **报告生成** | AI 协作撰写 | Word 手写 | 无 |
| **学习成本** | 低（引导式） | 高（需数学基础） | 中（需学习软件） |
| **使用时间** | 30 分钟 | 3-4 小时 | 2-3 小时 |

### 教育价值

- **降低学习门槛**：高中数学水平学生可独立完成实验报告，无需大学数值计算基础
- **强化误差意识**：可视化贡献分析直观展示哪个变量影响最大，激发改进实验的动机
- **鼓励实验创新**：节省报告时间，学生可投入更多精力设计改进实验或数据采集方法
- **适配教学改革**：支持翻转课堂与研究性学习，AI 作为"虚拟导师"讨论实验误差
...
<img src="icon.jpg" alt="作者头像" width="120" />

- 邮箱: quantum-liu@outlook.com
- GitHub: [quantumtino/Physics-Lab-Report-Assistant](https://github.com/quantumtino/Physics-Lab-Report-Assistant.git)
- 欢迎联系交流反馈！😊
